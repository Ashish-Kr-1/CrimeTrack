...
contact_num = '8679171050'
interactions = [r for r in records if r['B Party No'] == contact_num]

print(f"Total interactions with {contact_num}: {len(interactions)}")
for idx, r in enumerate(interactions):
    print(f"  {idx+1:02d}. [{r['Call Type']}] on {r['dt'].strftime('%Y-%m-%d %H:%M:%S')} (Dur: {r['Dur(s)']}s) via CGI {r['First CGI']} ({r['Roam Nw']})")
